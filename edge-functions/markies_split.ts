...
async function generateImageLoop(modelName: string, initialHistory: any[], imageConfig: any, baseBase64: string, refBase64: string, colorSwatchBase64: string | null, anchors?: { x_left: number; x_right: number; y: number; width_px: number }, fullBaseBase64?: string, cropBox?: { x: number; y: number; w: number; h: number }, globalStartMs: number = Date.now()) {
  ...
  for (let attempt = 1; attempt <= MAX_LOOPS; attempt++) {
    if (Date.now() - globalStartMs > 120000) break;
...
deno.serve(async (req: Request) => {
  ...
  try {
    ...
    */
    ...
    const promptObj = {
      role_definition: "You are an expert architectural image editor. You NEVER generate new scenes. You ONLY edit the provided Base Image.",
      task: "edit_image",
      description: "Add a Markies (Canopy) awning to the specific Base Image provided.",
      inputs: {
        base_image: {
          index: 0,
          description: "The first image (House). This is the ONLY image to be edited. The output MUST have the exact same pixels as this image, except for the awning.",
          role: "CANVAS"
        },
        reference_image: {
          index: 1,
      ...
      instructions: {
        ...
        appearance: {
          ...
          frame_color: `The frame hardware (housing, side ribs/slats) MUST be ${frame_color.toUpperCase()}.`,
      ...
      strict_preservation_rules: [
        "CRITICAL: The Base Image (Image 1) IS the output canvas. Do not change it.",
        "The output must be the Base Image with the awning added.",
        "Do NOT hallucinate a different house. If the house looks different, you have FAILED.",
        ...
        "Do NOT output the Reference Image. Output the Base Image with the features of the Reference Image applied."
      ]
    };
